...
const char* ssid = "Gopal"; //wifi user name
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("ESP8266Client")) {
      ...
      client.publish("yo","Hellooo world");                                           //acknowledgement
      // ... and resubscribe
      client.subscribe("yo");
    } else {
...
void loop() {
  ...
  client.loop();
    ...
    client.publish("Gopal/yo", temperatureTemp);
    ...
    if(t>30){
        delay(500);
        pinMode(4,HIGH);
       Serial.println("D2 is on");       //D2 is for fan or AC
      }
      else{
        delay(500);
        pinMode(4,LOW);
